...
public partial class SettingsPageViewModel(ISettingsService settingsService) : ObservableObject
{
    ...
    {
        get => ThemeModeOptions.IndexOf(settingsService.GetValue(SettingKeys.Theme, ElementTheme.Default));
        set
        {
            if(value == ThemeModeIndex)
                return;
            settingsService.SetValue(SettingKeys.Theme, ThemeModeOptions.GetAt(value).Key);
    ...
    {
        get => NavigationStyleOptions.IndexOf(settingsService.GetValue(SettingKeys.NavigationStyle, NavigationViewPaneDisplayMode.Left));
        set
        {
            if(value == NavigationStyleIndex)
                return;
            settingsService.SetValue(SettingKeys.NavigationStyle, NavigationStyleOptions.GetAt(value).Key);
    ...
    {
        get => LanguageOptions.IndexOf(settingsService.GetValue(SettingKeys.Language, LanguageSettings.System));
        set
        {
            if(value == LanguageIndex)
                return;
            var setting = LanguageOptions.GetAt(value).Key;
            settingsService.SetValue(SettingKeys.Language, setting);
